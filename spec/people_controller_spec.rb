...
describe "Our Person Controller" do
  ...
  before (:all) do
    ...
    @person = Person.create(firstname: @firstname, lastname: @lastname, birthdate: @birthdate)
  end
  ...
  it "displays a page with the person's name when we visit /people/:id" do
    ...
    expect(last_response.body.include?("#{@person.firstname} #{@person.lastname}")).to be(true)
  end  
  
  it 'creates a person when we post to /people' do
    people_count = Person.all.count
    post("/people", { firstname: "Kermit", lastname: "TheFrog", birthdate: (DateTime.now - 45.years) })
    expect(Person.all.count == people_count + 1).to eq(true)
  end
  
  it 'edits a person when we put to /people/:id' do
    new_first_name = "Mister"
    put("/people/#{@person.id}", { firstname: new_first_name, lastname: @lastname, birthdate: @birthdate })
    expect(@person.reload.firstname == new_first_name).to eq(true)
  end
